...
function perguntarNumeroMesa(proposta = "") {
  while (true) {
    const resposta = prompt(
      "Nenhuma mesa selecionada. Informe o número da mesa (1-999) para adicionar o produto:",
      proposta
    );
    if (resposta === null) return null;
    const numero = parseInt(resposta, 10);
    if (numero && numero > 0 && numero <= 999) return numero;
    alert("Digite um número de mesa válido entre 1 e 999.");
  }
}
...
function confirmarCriarMesaParaPedido(produto) {
  const numeroExistente = parseInt(numeroMesaInput.value, 10);
  if (numeroExistente && numeroExistente > 0 && numeroExistente <= 999) {
    if (!mesasAbertas.has(numeroExistente)) {
      const criar = confirm(
        `Nenhuma mesa selecionada. Deseja criar a mesa ${numeroExistente} e adicionar "${produto.nome}" a ela?`
      );
      if (!criar) return false;
      criarMesa(numeroExistente);
    } else {
      selecionarMesa(numeroExistente);
    }
    return true;
  }

  const novoNumero = perguntarNumeroMesa();
  if (!novoNumero) return false;
  criarMesa(novoNumero);
  return true;
}

function adicionarPedidoMesa(produto, triggerEl = null) {
  if (!mesaAtual) {
    if (!confirmarCriarMesaParaPedido(produto)) return;
  }
...
function getMesasFiltradas() {
  const termo = normalizarTexto(mesaSearchTerm || "").trim();
  ...
  return mesas.filter(([numero, mesa]) => {
    ...
    return mesa.pedidos.some((pedido) => {
      const textoPedido = [pedido.nome, pedido.categoria, pedido.comentario]
        .map(normalizarTexto)
        .join(" ");
      return textoPedido.includes(termo);
    });
  });
}
